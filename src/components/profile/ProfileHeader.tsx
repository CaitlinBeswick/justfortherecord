...
export const ProfileHeader = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: listened,
        error: listenedError
      } = await supabase.from('listening_status').select('release_group_id').eq('user_id', user!.id).eq('is_listened', true);
      ...
      const {
        data: rated,
        error: ratedError
      } = await supabase.from('album_ratings').select('release_group_id').eq('user_id', user!.id);
      if (ratedError) throw ratedError;

      // Merge and deduplicate by release_group_id
      const uniqueIds = new Set([...(listened?.map(l => l.release_group_id) || []), ...(rated?.map(r => r.release_group_id) || [])]);
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('artist_follows').select('id').eq('user_id', user!.id);
      if (error) throw error;
      return data;
    },
    enabled: !!user
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('friendships').select('id').eq('status', 'accepted').or(`requester_id.eq.${user!.id},addressee_id.eq.${user!.id}`);
      if (error) throw error;
      return data;
    },
    enabled: !!user
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('diary_entries').select('listened_on').eq('user_id', user!.id);
      if (error) throw error;
      return data;
    },
    enabled: !!user
  ...
  const thisYearCount = diaryEntryDates.filter(entry => isAfter(new Date(entry.listened_on), thisYearStart) || new Date(entry.listened_on).getFullYear() === currentYear).length;
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 relative">
        ...
        duration: 0.4
          ...
          <div className="flex flex-col items-center text-center">
            {/* Avatar */}
            {profile?.avatar_url ? <img src={profile.avatar_url} alt="Profile" className="w-28 h-28 md:w-32 md:h-32 rounded-full object-cover border-4 border-border/50" /> : <div className="w-28 h-28 md:w-32 md:h-32 rounded-full bg-secondary flex items-center justify-center border-4 border-border/50">
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <button onClick={() => navigate("/profile/settings")} className="flex h-8 w-8 items-center justify-center rounded-lg bg-secondary text-muted-foreground transition-colors hover:bg-surface-hover hover:text-foreground" title="Settings">
                <Settings className="h-4 w-4" />
              </button>
            </div>

            {/* Bio */}
            {profile?.bio && <p className="text-muted-foreground mt-2 max-w-lg">{profile.bio}</p>}
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 mt-6">
              ...
              {/* Listening Goal */}
              {profile?.yearly_listen_goal && thisYearCount !== undefined && <div className="flex items-center gap-3 pl-6 border-l border-border/50">
                  ...
                  <div className="min-w-[80px] text-left">
                    ...
                    <Progress value={Math.min(thisYearCount / profile.yearly_listen_goal * 100, 100)} className="h-1.5 mt-1" />
