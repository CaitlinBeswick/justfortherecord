...
import { Disc3, Users, Trophy, Star, ArrowRight, Eye, EyeOff } from "lucide-react";
...
const DiscoveryLeaderboards = () => {
  ...
  const { allStatuses } = useListeningStatus();

  // Fetch user's artist ratings to know which artists they've rated
  ...
  const isAlbumListened = (releaseGroupId: string) => {
    return allStatuses.some(
      (s) => s.release_group_id === releaseGroupId && s.is_listened
    );
  };
  ...
  const isLoading = activeTab === "albums" ? topAlbumsLoading : topArtistsLoading;
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="font-serif text-3xl md:text-4xl text-foreground">
                  Top 250 {activeTab === "albums" ? "Albums" : "Artists"}
                </h1>
              </div>
              <p className="text-muted-foreground">
                The highest rated {activeTab} by the community
              </p>
            </motion.div>
